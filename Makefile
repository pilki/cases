...
COQLIBS:=-I src -R theories Constructors
COQDOCLIBS:=-R theories Constructors
...
all: $(VOFILES) $(CMOFILES) $(CMXSFILES) src/constructors_plugin.cmxs\
  src/constructors_plugin.cma
spec: $(VIFILES)
...
src/constructors_plugin.cmxs: src/constructors.cmx src/constructors_plugin_mod.cmx
	$(CAMLOPTLINK) $(ZDEBUG) $(ZFLAGS) -shared -o src/constructors_plugin.cmxs src/constructors.cmx src/constructors_plugin_mod.cmx

src/constructors_plugin.cma: src/constructors.cmo src/constructors_plugin_mod.cmo
	$(CAMLLINK) -g -a -o src/constructors_plugin.cma src/constructors.cmo src/constructors_plugin_mod.cmo

%.vo %.glob: %.v $(CONSTRUCTORS_PLUGINOPT) $(CONSTRUCTORS_PLUGIN)
...
install:
	...
	(cd theories; for i in $(VOFILES1); do \
	 install -d `dirname $(COQLIB)/user-contrib/Constructors/$$i`; \
	 install $$i $(COQLIB)/user-contrib/Constructors/$$i; \
	 done)
...
clean:
	...
	- rm -rf html
	- rm -f src/constructors_plugin.cmxs
	- rm -f src/constructors_plugin.cma
